...
class FileUtilTest {
    ...
    void delete() {
        ...
        // 测试删除单个文件
        boolean ok1 = FileUtil.delete(property+"/tmp/testdir/abc.txt");
        System.out.println("删除单个文件结果: " + ok1);

        // 测试删除目录（含递归删除子文件）
        boolean ok2 = FileUtil.delete(property+"/tmp/testdir/dir1");
        System.out.println("删除目录结果: " + ok2);

        // 测试空路径
        boolean ok3 = FileUtil.delete("   ");
        System.out.println("删除空路径: " + ok3);

        // 测试不存在的文件
        boolean ok4 = FileUtil.delete(property+"/tmp/testdir/not-exist.txt");
        System.out.println("删除不存在: " + ok4);

        // 测试传入null
        boolean ok5 = FileUtil.delete((File)null);
